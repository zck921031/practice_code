...
struct IP{
    ...
    bool operator < ( const IP t ) const{
        //return ( mask!=t.mask ? mask<t.mask : addr<t.addr );
        return ( addr!=t.addr ? addr<t.addr : mask<t.mask );
    }
    bool exist(const IP t)const{
        if( (mask > t.mask) && ( t.addr>>(32-t.mask)==addr>>(32-t.mask) ) ) return true;
        else return false;
    }
    bool can_merge(const IP t)const{
        if( (mask == t.mask) && ( (t.addr^addr)==( 1LL )<<(32-mask) ) ) return true;
        else return false;
    }
};
IP ip[100001];
...
//    for(int i=0; i<N; i++){
//        cout<<ip[i].to_str()<<endl;
//    }
    set<IP> vec[33], tree;
    for(int i=0; i<N; i++){
        int mask = ip[i].mask;
        vec[mask].insert( ip[i] );
        tree.insert( ip[i] );
    }
    set<IP> ans;
    for (int mask=32; mask>=0; mask--){
        for(IP t : vec[mask] ){
            bool skip = false;
            for(IP s : tree){
                if ( t.addr==s.addr && t.mask==s.mask  ) continue;
                if ( t.mask && t.exist( s ) ){
                    skip=1;
                    continue;
                }
                if ( t.mask && t.can_merge(s) ){
                    skip=1;
                    IP next;
                    next.addr = s.addr&t.addr;
                    next.mask = t.mask - 1;
                    vec[ mask-1 ].insert(next);
                    tree.insert( next );
                    continue;
                }
            }
            if ( !skip ) ans.insert(t);
        }
    }
    for( IP t:ans ){
        cout<<t.to_str()<<endl;
