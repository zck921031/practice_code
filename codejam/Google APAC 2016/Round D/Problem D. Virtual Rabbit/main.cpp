...
//}
const int secondsoneday = 24*60*60;
int RDtime(){
    string str;
    cin>>str;
    int h,m,s;
...
inline bool canfeed(LL t, int G, int W, int H, int B){
    if (t<0) return false;
    if (0==t) return true;
    t = t%secondsoneday;
    return ( (G<=t&&t<W) || (H<=t&&t<B) )?true:false;
}
void run(){
    int D, G, W, H, B, X;
    G = RDtime();
    ...
    X = RDtime();
    cin>>D;
    /// during time [G, W) and [H, B), girl can feed the pet.
    /// during time [W, H) and [B, G), girl can not feed the pet.
    //cout<<G<<" "<<W<<" "<<H<<" "<<B<<" "<<X<<" | "<<max(H-W, secondsoneday-B+G)<<endl;return ;
    if ( X <= max(H-W, secondsoneday-B+G) ){
        cout<<-1<<endl;
        return ;
    }
    long long tick = D*secondsoneday;
    long long cnt = 0;
    while( tick>0 ){
        long long pre = tick;
        for(int i=1; i<=X; i++){
            if ( canfeed(tick-i, G,W,H,B) ) pre = tick-i;
            //cout<<canfeed(tick-i, G,W,H,B)<<endl;
        }
//        cout<<pre%secondsoneday/3600<<":"<<pre%3600/60<<":"<<pre%60<<endl;
//        cout<<canfeed(8*3600, G,W,H,B)<<endl;
//        cout<<canfeed(pre, G,W,H,B)<<endl;
//        cout<<canfeed(pre-1, G,W,H,B)<<endl;
        tick = pre;
        if(tick>0) cnt++;
    }
    cout<<cnt<<endl;
