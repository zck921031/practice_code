...
void run(){
    LL A, B, N, K;
    cin>>A>>B>>N>>K;
    vector<LL> ha = vector<LL>(K,0);
    vector<LL> hb = vector<LL>(K,0);
    LL sub = 0;
    LL n, mu;
    n = N % K;
    mu = (N / K) % mod;
    for(LL i=1; i<=K; i++){
        ...
        (hb[tb] += mu) %= mod;
        LL temp_sub = 0;
        if ( (ta+tb)%K == 0 ) temp_sub += mu;
        if( i <= n ){
            (ha[ta]+=1)%= mod;
            (hb[tb]+=1)%= mod;
            if ( (ta+tb)%K == 0 ) temp_sub += 1;
        }
        (sub += temp_sub) %= mod;
    }
    LL ans = 0;
    for(LL i=0; i<K; i++){
        (ans+= ha[i]*hb[ (K-i)%K ]) %= mod;
//        cerr<<ha[i]<<","<<hb[i]<<endl;
    }
    cerr<<ans<<" "<<sub<<" "<<(ans-sub+mod) % mod<<endl;
    ans = (ans-sub+mod) % mod;
    cout<<ans<<endl;
}
int main(){
