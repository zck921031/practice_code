...
struct BIT{
    ...
    map<LL, LL> c1;    /* 维护delta[i]的前缀和 */
    map<LL, LL> c2;    /* 维护delta[i]*i的前缀和 */
    map<LL, LL> Sum;
    LL n;
    void init(LL _n){
        n=_n;
    ...
    void update(map<LL, LL> &array, LL i, LL d){
        while (i <= n) {
            array[i] += d;
            i += lowbit(i);
        }
    }
    void add(LL s, LL t, LL d)//将区间[s,t]所有元素加上d
    {
...
void run(){
    ...
    for(int i=0; i<N; i++){
        L[i] = min(x[i], y[i]) + bias;
        R[i] = max(x[i], y[i]) + bias;
        events.push_back( make_pair(L[i], R[i]) );
        tick[ L[i] ] += 1;
        tick[ R[i]+1 ] -= 1;
//        cerr<<L[i]<<" "<<R[i]<<endl;
    }
...
//        cerr<<key<<" "<<value<<endl;
        if( cover > 0 ){
            ...
            int b = key-1;
//            if (!p.count(cover)){
//                p.insert( make_pair(cover, vector<PII>()) ) ;
//            }
//            p[cover].push_back( make_pair(a,b) );
//            cerr<<a<<"-"<<b<<" : "<<cover<<endl;
            sum += b-a+1;
            if ( 1 == cover ){
                s.add(a, b, 1);
