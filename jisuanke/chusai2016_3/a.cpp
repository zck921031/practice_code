...
long long hashhash[MAXN], power[MAXN];
bool check[MAXN];
char str[MAXN];
//S[1..n]
int query(int a, int b){
    a --;
    int len = b - a, res = 1;
    for (int j = 0; j < (int)prime[len].size(); j ++) {
        int p = prime[len][j].first, cnt = prime[len][j].second, now = 1;
        for (int k = 0; k < cnt; k ++) now *= p;
        for (int k = cnt; k > 0; k --) {
            int l = len / now;
            long long t1 = hashhash[b-l] - hashhash[a] * power[b-a-l];
            long long t2 = hashhash[b] - hashhash[a+l] * power[b-a-l];
            if (t1 == t2) {
                res *= now;
                break;
            }
...
int main() {
	...
	hashhash[0] = 0;
	for (int i = 0; i < n; i ++)
		hashhash[i+1] = hashhash[i] * base + str[i];
	power[0] = 1;
	for (int i = 1; i < n; i ++)
