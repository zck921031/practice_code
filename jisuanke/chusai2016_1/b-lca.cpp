...
public:
    ...
    void init(int n){
        ...
        int i, j;
        for( d[0]=1, i=1; i < M; ++i ) d[i] = 2*d[i-1];
        ...
        for( j=1; j < k; ++j )
            for( i=0; i < n; ++i ){
                if( i+d[j-1]-1 < n ){
                    st[i][j] = min(st[i][j-1], st[i+d[j-1]][j-1], comp);
                }
                else break; // st[i][j] = st[i][j-1];
            }
    }

    inline T query(int x, int y ){   // x, y range:0...n-1
        int k;
        k = Ln[ y-x+1 ];
        Comp comp;
        return( min(st[x][k], st[y-d[k]+1][k], comp) );
    }
};

RMQ< int > r;
//RMQ< int , greater<int> > rmqmax;

int tmd[MAXN], itm[MAXN], tot;//tmd[]先序时间戳，itm[]时间戳->顶点序号,r是RMQ类求最小值,dfs后调用r.init(tot);tot从1开始
vector<int> e[MAXN];
int depth[MAXN];
...
inline int lca(int u, int v){
    int tu=tmd[u], tv=tmd[v];
    if (tu>tv) swap(tu,tv);
    int ret=r.query(tu, tv);
    return itm[ret];
}
void run(int n){
    for(int i=1; i<=n; i++){
...
//            if( coprime[i/100+1][j/100+1] ){
//                //cout<<a[i]<<" "<<a[j]<<" "<<dis[i][j]<<endl;
//                sum += 1;
//            }
//        }
//    }
